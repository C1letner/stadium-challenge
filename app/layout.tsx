...
function LogoMark({ size = 36 }: { size?: number }) {
  ...
  return (
    <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} fill="none">
      <polygon points={`${cx},${top} ${left},${mid} ${right},${mid}`} stroke="#3b82f6" strokeWidth="2" fill="none"/>
      <polygon points={`${cx},${top + s*0.1} ${m1l},${mid} ${m1r},${mid}`} fill="#1e293b" stroke="#60a5fa" strokeWidth="1"/>
      <polygon points={`${cx},${top + s*0.2} ${m2l},${mid} ${m2r},${mid}`} fill="#3b82f6"/>
      <polygon points={`${cx},${top + s*0.3} ${m3l},${mid} ${m3r},${mid}`} fill="#60a5fa"/>
      <line x1={stickX} y1={stickBottom} x2={stickX} y2={stickTop} stroke="#38bdf8" strokeWidth="2" strokeLinecap="round"/>
